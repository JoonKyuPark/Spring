...
<script type="text/javascript">
   window.onload = function() {
      ...
      $("#member").on("click", function() {
         $("#member").addClass("person");
         $("#etp").removeClass("corp");
      });
      $("#etp").on("click", function() {
         $("#etp").addClass("corp");
         $("#member").removeClass("person");
      });
   ...
   var id_check = function() {
      ...
      $.getJSON("/mem/id_check" + $("#mem_idCheck").val(), function(result) {

         if (result == "0") {
            alert("사용할수 있는 아이디 입니다")
            $("#mem_check").attr("checked", true);
         } else {
   ...
   var id_check2 = function() {

      $.getJSON("/etp/id_check" + $("#etp_idCheck").val(), function(result) {
...
① "사이트"라 함은 회사가 서비스를 "회원"에게 제공하기 위하여 컴퓨터 등 정보통신설비를 이용하여 설정한 가상의 영업장 또는 회사가 운영하는 웹사이트, 모바일웹, 앱 등의 서비스를 통칭하며, 통합된 하나의 회원 계정(아이디 및 비밀번호)을 이용하여 서비스를 제공받을 수 있는 아래의 사이트를 말한다.
- www.jobkorea.co.kr
- www.albamon.com
- www.hrpartners.co.kr
- www.devjob.co.kr
- www.campusmon.com
- www.jobbusan.co.kr
- m.jobkorea.co.kr
...
③ "회원"이라 함은 자사의 직접 고용의 목적으로 채용을 원하는 기업이 본 서비스를 이용하기 위하여 동 약관에 동의하고 "회사"와 이용계약을 체결하여 아이디를 부여받은 "기업회원"을 말하며 자사의 직접 고용을 목적으로 하지 않는 헤드헌팅, 아웃소싱(도급), 인재파견, 채용대행 등 인력 중개를 주 목적으로 사업을 수행하는 업체는 별도의 "서치펌회원" 또는 "파견대행회원"으로 가입하여야 한다.
...
④ "아이디"라 함은 회원의 식별과 회원의 서비스 이용을 위하여 회원이 선정하고 "회사"가 부여하는 문자와 숫자의 조합을 말한다.
⑤ "비밀번호"라 함은 "회사"의 서비스를 이용하려는 사람이 아이디를 부여받은 자와 동일인임을 확인하고 "회원"의 권익을 보호하기 위하여 "회원"이 선정한 문자와 숫자의 조합을 말한다.
⑥ "비회원"이라 함은 "회원"에 가입하지 않고 "회사"가 제공하는 서비스를 이용하는 자를 말한다.
제 3 조 (약관의 명시와 개정)
...
② 제1항 신청에 있어 "회사"는 "회원"의 종류에 따라 전문기관을 통한 실명확인 및 본인인증을 요청할 수 있다. "회원"은 본인인증에 필요한 이름, 생년월일, 연락처 등을 제공하여야 한다, 또한 서비스 이용을 위해 회사명과 사업자등록번호를 제공하여야 하며 "회사"는 기업인증을 위해 사업자정보를 확인할 수 있는 증명서(사업자등록증)를 별도로 요구할 수 있다. 회사명과 사업자등록번호를 허위로 등록한 경우, 휴·폐업 사업자등록번호로 등록한 경우에 "회원"은 일체의 권리를 주장할 수 없다.
...
① "회원"은 구직을 원하는 개인회원이 채용공고를 열람한 후 해당 기업에 대한 판단을 돕기 위해 기업정보를 정확히 기재하여야 한다.
...
③ "회원"은 관련 법률에 의거하여 근로자의 모집 및 채용에 있어서 남녀, 연령을 차별하여서는 아니되며 15세 미만 채용 모집을 할 수 없다.
④ 제2항, 제3항을 위반한 경우 이에 대한 책임은 전적으로 "회원"에게 있다.
...
⑦ "회원"이 제2조 제 3항에 따라 자사의 직접 고용을 목적으로 하지 않는 헤드헌팅, 아웃소싱(도급), 인재파견, 채용대행 등 인력 중개를 목적으로 하는 채용공고를 등록할 경우 "회사"는 해당 공고를 삭제할 수 있으며 "기업회원"의 자격을 "서치펌회원 또는 파견·대행회원"으로 전환시킬 수 있다. 단, 인력 중개를 목적으로 하는 채용공고의 경우에도 제2조 제3항의 예외 기준에 따라 가입된 "기업회원"에 한해서는 예외로 하며, 알바몬(www.albamon.com)의 경우 직접 고용 여부와 상관없이 “기업회원”으로 이용할 수 있다.
제 9 조 (제휴를 통한 서비스)
① "회사"는 제휴 관계를 체결한 여타 인터넷 웹사이트 및 채용박람회 또는 신문, 잡지 등의 오프라인 매체를 통해 사이트에 등록한 "회원"의 기업정보와 채용공고가 열람될 수 있도록 서비스를 제공할 수 있다.
...
1. 워크넷 http://www.work.go.kr
④ "제9조 ③호" 제휴를 통한 사이트의 변동사항 발생 시 공지사항을 통해 고지 후 진행 한다.
제 10 조 (서비스의 요금)
① "회원" 가입과 채용공고 등록비용은 무료이다. 다만 개인회원 또는 사이트에 방문한 회원에게 자신의 정보를 보다 효과적으로 노출시키기 위한 유료옵션과 즉시등록 서비스, 유료 이력서 검색 및 기타 서비스를 이용하기 위한 별도의 서비스는 유료로 제공될 수 있다.
...
제 11 조 (e-머니 및 충전금 규정)
① e-머니는 유료상품 구매 시의 사은, 이벤트, 고객보상 등의 사유로 적립된다.'적립된 e-머니'는 유료상품 결제시 사용할 수 있으며, 현금으로 출금되지 않는다.
② 위의 적립사유가 해제되었을 경우, ‘적립된 e-머니’를 회원보유분에서 회수하며, 결제시 사용한 서비스를 취소할 경우, 사용분에 해당하는 e-머니를 제외하고 사용이전의 상태로 재적립 받을 수 있다.
...
④ e-머니의 적립시점과 무관하게 유효기간이 지나거나, "회원"이 12개월 동안 사이트에 로그인 하지 않을 경우, "회원"이 가입해지한 경우, 제19조 ②항에 의해 가입해지하거나 서비스가 중지된 경우 e-머니는 전액 자동소멸되어 복구되지 않는다.
⑤ "회원"인 개인사업자와 법인사업자의 휴·폐업 여부가 회사가 정한 별도의 절차에 따라 확인된 경우 적립된 e-머니는 전액 소멸된다.
...
⑧ "회원"은 "회사"에 잔여 충전금의 환불을 요청할 수 있으며, "회사"는 "회원"에게 환불에 필요한 서류를 요청할 수 있다. 환불 시 환불수수료를 공제하고 환불할 수 있으며, 충전금 충전방법에 따라 금융비용이 환불수수료를 초과하여 발생할 경우 이는 "회원"의 부담으로 한다.
...
① 자료내용은 "회원"이 등록한 기업정보 및 채용공고와 사이트에 게시한 게시물을 말한다.
...
① "회원"이 입력한 정보는 취업 및 관련 동향의 통계 자료로 활용될 수 있으며 그 자료는 매체를 통해 언론에 배포될 수 있다.
...
③ "회원"이 결제 수단으로 신용카드를 사용할 경우 비밀번호 등 정보 유실 방지는 "회원" 스스로 관리해야 한다. 단, "사이트"의 결함에 따른 정보유실의 발생에 대한 책임은 회원의 의무에 해당하지 아니한다.
...
17. 제2조 제3항의 예외 기준에 따라 가입된 "기업회원"이 아닌 "회원"이 자사의 직접 고용을 목적으로 하지 않는 헤드헌팅, 아웃소싱(도급), 인재파견, 채용대행 등 인력 중개를 주 목적으로 사업을 수행하는 업체의 경우
18. 다단계, 피라미드 식의 모집 내용을 등록했을 때
19. 구인을 가장하여 유학알선, 물품판매, 자금 모금 등을 행하는 내용을 등록했을 때
20. 회원모집 형태의 광고 또는 카드회원 모집(수당제 광고) 내용을 등록했을 때
21. 추천인 모집 광고, 재택 쇼핑몰 분양 광고, 직업소개, 부업알선 광고를 등록했을 때
22. 취업수수료 등 취업 관련 비용을 필요로 하는 채용공고를 등록했을 때 (운송, 물류, 지입, 인력용역 등)
23. 불건전한 내용의 유흥업소 채용공고를 등록했을 때
24. 구직자의 피해사례 (채용공고를 통해 구직자가 입사 과정에서 피해가 발생한 경우 또는 재직중 "기업회원"이 관계법령을 위반하여 피해가 발생한 경우 등)가 접수된 채용공고
25. 임금체불로 근로감독원에 진정서가 접수되거나, 관계 당국에 고소·고발되어 있는 기업 (단, 기업의 임금체불이 해소가 입증되면 확인 후에 서비스 이용 가능)
26. 본 서비스 명예를 훼손하였을 때
27. 동업자 모집, 프랜차이즈 모집, 점포개설, 창업, 사이트 홍보 등 직원 채용이 아닌 공고를 등록했을 때
28. 기타 관계법규에 위배되는 내용이거나 사이트의 원활한 운영을 위하여 운영자가 필요하다고 판단한 경우
③ "회원"이 유료서비스를 이용하는 중 "회사"의 책임에 의해 정상적인 서비스가 제공되지 않을 경우 "회원"은 본 서비스의 해지를 요청할 수 있으며 "회사"는 기간의 정함이 있는 유료서비스의 경우에는 해지일까지 이용일수를 1일 기준금액으로 계산하여 이용금액을 공제후 환급하고, 이용건수의 정함이 있는 유료서비스의 경우에는 기 사용분을 1건 기준금액으로 계산하여 이용금액을 공제후 환급한다. 단, 허위·불량공고의 경우 게재시 지불된 광고료는 일체 반환되지 않는다.
④ "회원"인 개인사업자와 법인사업자의 휴·폐업 여부가 회사가 정한 별도의 절차에 따라 확인된 경우 유료 이력서 검색서비스(건수별 인재검색)의 경우 적립된 건수는 모두 소멸된다.
...
③ "회원"이 이 약관의 제5조, 제18조, 제19조, 제20조 등의 규정에 위반한 행위로 "회사" 및 제3자에게 손해를 입히거나 "회원"의 책임 있는 사유에 의해 "회사" 및 제3자에게 손해를 입힌 경우에는 "회원"은 그 손해를 배상하여야 한다.
...
① 과다납입한 요금에 대하여는 그 금액을 반환한다. 다만, "회원"이 동의할 경우 다음 달에 청구할 요금에서 해당 금액만큼을 감하여 청구한다.
② 요금을 반환받아야 할 "회원"이 요금체납이 있는 경우에는 반환해야 할 요금에서 이를 우선 공제하고 반환한다.
③ "회사"는 과소청구액에 대해서는 익월에 합산청구한다.
제 23 조 ("회원"의 개인정보보호)
...
유료 서비스 이용 과정에서 아래와 같은 결제 정보들이 수집될 수 있습니다. 신용카드 결제: 카드사명, 카드번호 등
은행계좌 이체: 은행명, 계좌번호 등
   ...
   <div class="tab-content">
      <div class="tab-pane active" id="person">
         <form action="join/mem" id="member_join" method="post">
            ...
            <br>
            <div class="container">
               <button class="col-md-1" onclick="check(event)">회원가입</button>
      ...
      <div class="tab-pane" id="coper">
         <form action="mypage/etp/join/etp" id="etp_join" method="post">
            ...
            <div class="row">
               ...
               <div class="col-md-1">
                  <input type="etp_tel" required="required"
                     onkeyup="this.value=this.value.replace(/\D/,'')"
                     style="ime-mode: disabled;">
               </div>
               <div class="col-md-1">
		...
		<div class="tab-pane active" id="person">
			<form action="join/mem" id="member" method="post">
				...
				<div class="container">
					...
					<div class="col-md-1">
						<input type="button" value="중복체크" onclick="id_check()"> <input
							type="checkbox" id="mem_check">
					</div>
		...
		<div class="tab-pane" id="coper">
			<form action="/join/etp" id="etp" method="post">
				...
				<div class="container">
					...
					<div class="col-md-1">
						<input type="button" value="중복체크" onclick="id_check2()"> <input
							type="checkbox" id="etp_check">
