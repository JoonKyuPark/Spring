		...
		<tbody>
			<c:forEach var="i" begin="0" end="${recruit_list.size()-1}" step="1">
				<tr height="30">
					<td align="center">
						<div class="checks etrans">
							<input type="checkbox" id="${recruit_list[i].recruit_no}" name="box">
							<label for="${recruit_list[i].recruit_no}"></label>
						</div>
					</td>
					<td align="center"><h4>${etp_read_list[i].etp_name}</h4></td>
					<td align="left"><h4>
							<a href="/recruit/mem_Recruit_Read_Form?rno=${recruit_list[i].recruit_no}">${recruit_list[i].recruit_title}</a>
						</h4> <a href="../Member_Recruit_Clip/Member_Recruit_ClipOk.jsp?id=${recruit_list[i].recruit_no}">좋아요</a></td>
